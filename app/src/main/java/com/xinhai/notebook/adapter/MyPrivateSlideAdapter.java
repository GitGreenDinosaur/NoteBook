...
public class MyPrivateSlideAdapter extends RecyclerView.Adapter<MyPrivateSlideAdapter.SlideViewHolder> {

    private final List<Note> mNoteList;
    private final String TAG = "MyPrivateSlideAdapter";

    public MyPrivateSlideAdapter(List<Note> notes) {
        mNoteList = notes;
    }
    ...
    public SlideViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(
                R.layout.item_layout_private_rv, parent, false);
    ...
    public void onBindViewHolder(@NonNull SlideViewHolder holder, int position) {
        ...
        if (!holder.mDeleteTv.hasOnClickListeners()) {
            holder.mDeleteTv.setOnClickListener(v -> {
                ...
                //弹出提醒弹窗
                RecoverDialog removeDialog
                        = new RecoverDialog(v.getContext(), R.style.MyDialog);
                removeDialog.show();
                removeDialog.setOnDismissListener(d -> {
                    if (SpfUtil.getString(Constant.FLAG_DIALOG_CALLBACK).equals("true")) {
                        //1.移除数据库
                        DBManager.removeItemToWasterTab(mNoteList.get(position)
                                , Constant.TABLE_NAME_PRIVATE);
                        //2.删除列表内容
        ...
        if (!holder.mUnPrivateTv.hasOnClickListeners()) {
            holder.mUnPrivateTv.setOnClickListener(v -> {
                DBManager.recoverItemToNoteTab(mNoteList.get(position));
                mNoteList.remove(holder.getAdapterPosition());
                notifyItemRemoved(position);//移除当前位置
                MyToastUtil.success("取消成功");
            });
        }
        ...
        holder.mGlobalLl.setOnClickListener(v -> {
            ...
            intent.putExtra(Constant.EDIT_STATUS,Constant.NOTE_STATUS_PRIVATE); //给编辑界面，传一个编辑的状态
    ...
    public static class SlideViewHolder extends RecyclerView.ViewHolder {
        ...
        private final TextView mUnPrivateTv;
        private final LinearLayout mGlobalLl;

        private SlideViewHolder(@NonNull View itemView) {
            super(itemView);
            mTitleTv = itemView.findViewById(R.id.item_private_tv_title);
            mContentTv = itemView.findViewById(R.id.item_private_tv_content);
            mTimeTv = itemView.findViewById(R.id.item_private_tv_time);
            mDeleteTv = itemView.findViewById(R.id.item_private_tv_delete);
            mUnPrivateTv = itemView.findViewById(R.id.item_private_tv_unPrivate);
            mGlobalLl = itemView.findViewById(R.id.item_private_ll_global);
